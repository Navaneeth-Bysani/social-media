...
exports.signup = catchAsync(async(req,res,next)=> {
	...
	if(userCheck) return next(new AppError('User already exists',500));
...
exports.confirmSignup = catchAsync(async(req,res,next) => {
	...
	if(newUser.confirmEmailExpires < Date.now()) {
		return next(new AppError('Token is expired! Please signup again',500))
	}
